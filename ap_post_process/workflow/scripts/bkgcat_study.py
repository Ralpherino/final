...
########################################################################
# Summary tables for detailed / simplified / pretty categories
########################################################################

def _count_actions_to_dict(named_actions):
    """Run multiple RDF Count() actions together and return {name: count}."""
    handles = [h for _, h in named_actions]
    if handles:
        ROOT.RDF.RunGraphs(handles)
    return {name: int(h.GetValue()) for name, h in named_actions}


def _counts_dict_to_df(counts, sort=True):
    df = pd.DataFrame(
        [{"Category": k, "count": v} for k, v in counts.items()]
    )
    total = int(df["count"].sum()) if len(df) else 0
    if total > 0:
...
def build_categorized_rdf(
    ...
    """
    Build the RDF with all category columns and grouped labels.
    """
    return add_categories_and_filter(
        rdf,
        apply_BDT_Iso_cut=apply_BDT_Iso_cut,
        apply_PIDK_cut=apply_PIDK_cut,
        apply_Xc_Selection_cut=apply_Xc_Selection_cut,
    )


def detailed_category_table(
    rdf,
    apply_BDT_Iso_cut=True,
    apply_PIDK_cut=True,
    apply_Xc_Selection_cut=False,
    ...
    """
    Table like the detailed categorisation screenshot.
    """
    rdf_cat = build_categorized_rdf(
        rdf,
        apply_BDT_Iso_cut=apply_BDT_Iso_cut,
        apply_PIDK_cut=apply_PIDK_cut,
        apply_Xc_Selection_cut=apply_Xc_Selection_cut,
    ...
    named_actions = []
    for idx, name in categories_map.items():
        if idx == -1:
            continue
        h = rdf_cat.Filter(f"category == {idx}", name).Count()
        named_actions.append((name, h))

    counts = _count_actions_to_dict(named_actions)
    return _counts_dict_to_df(counts, sort=True)


def simplified_category_table(
    rdf,
    apply_BDT_Iso_cut=True,
    apply_PIDK_cut=True,
    apply_Xc_Selection_cut=False,
    ...
    """
    Table in simplified-category names:
    normlike, doubleCharm, Xc_background, ...
    """
    rdf_cat = build_categorized_rdf(
        rdf,
        apply_BDT_Iso_cut=apply_BDT_Iso_cut,
    ...
    named_actions = []
    for idx, name in simplified_categories_map.items():
        if idx == -1:
            continue
        h = rdf_cat.Filter(f"simplified == {idx}", name).Count()
        named_actions.append((name, h))
...
def pretty_category_table(
    ...
    """
    Table like the thesis-style grouped summary:
    Normalization like, Double Charm, Bad Xc, ...
    """
    simp_df = simplified_category_table(
        rdf,
        apply_BDT_Iso_cut=apply_BDT_Iso_cut,
        apply_PIDK_cut=apply_PIDK_cut,
        apply_Xc_Selection_cut=apply_Xc_Selection_cut,
    ...
    # exclude the total row before regrouping
    simp_df = simp_df[simp_df["Category"] != "total"].copy()

    pretty_counts = {}
    for _, row in simp_df.iterrows():
        simple_name = row["Category"]
        pretty_name = pretty_bkg_name_map[simple_name]
        pretty_counts[pretty_name] = pretty_counts.get(pretty_name, 0) + int(row["count"])

    return _counts_dict_to_df(pretty_counts, sort=True)


def print_detailed_category_table(
    rdf,
    apply_BDT_Iso_cut=True,
    apply_PIDK_cut=True,
    apply_Xc_Selection_cut=False,
):
    df = detailed_category_table(
        rdf,
        apply_BDT_Iso_cut=apply_BDT_Iso_cut,
        apply_PIDK_cut=apply_PIDK_cut,
        apply_Xc_Selection_cut=apply_Xc_Selection_cut,
...
def print_pretty_category_table(
    rdf,
    apply_BDT_Iso_cut=True,
    apply_PIDK_cut=True,
    apply_Xc_Selection_cut=False,
):
    df = pretty_category_table(
        rdf,
        apply_BDT_Iso_cut=apply_BDT_Iso_cut,
        apply_PIDK_cut=apply_PIDK_cut,
        apply_Xc_Selection_cut=apply_Xc_Selection_cut,
